...
function createHappyPathDomesticContactInfo(contactNumber) { createContactInfo(contactNumber,true);}
...
function createRandomUserInfo(randomizePassword = false) {
    ...
    pm.environment.set("user:name", name);
    pm.environment.set("user:email", email);
    pm.environment.set("user:password", randomizePassword ? pm.variables.replaceIn("{{$randomPassword}}") : "Passw@rd1");
}
